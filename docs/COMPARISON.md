...
| 智能分类 | karakeep 等依赖服务端 AI 打标签，数据需外发 | 可选 AI 分类，且只发送裁剪后的 id/标题/URL；未配置时完全本地 | 
| 归档预演 | 少见 | 执行前生成 operations / folders / conflicts 预览，可校验结果 |
| 分类可解释 | 通常是黑盒 | 本地分类器返回打分理由；AI 分类返回置信度、标签与判定理由 |
...
| AI 自动打标签（强制） | 不强制外发：默认全本地，AI 分类为可选，且只发送最小字段 |
